...
const App = () => {
  // Vom păstra toate adresele într-un singur array.
  ...
  // Funcția callback pentru adăugarea unei adrese din AutoCompleteInput
  const addAddress = (coordsWithLabel) => {
    setAddresses((prev) => [...prev, coordsWithLabel]);
  };

  // Funcții pentru reordonarea și ștergerea adreselor din listă
  const moveUp = (index) => {
    if (index === 0) return; // nu putem muta primul element mai sus
  ...
  const moveDown = (index) => {
    if (index === addresses.length - 1) return; // nu putem muta ultimul element mai jos
    const newArr = [...addresses];
    const temp = newArr[index];
    newArr[index] = newArr[index + 1];
    newArr[index + 1] = temp;
  ...
  const getRoute = async () => {
    ...
    // Extragem datele: prima adresă = start, ultima = destinație, intermediare = între ele
  ...
  // Funcția pentru afișarea rutei pe hartă
  ...
  // Callback primit de la TollCalculator
  ...
  // Se execută la submit (calculăm ruta)
  ...
  return (
    <div className="App">
      ...
      <div className="container">
        <div className="sidebar">
          <form onSubmit={handleSubmit}>
            {/* O singură casetă text pentru introducerea adreselor */}
            <label>
              Introdu adresa:
              <AutoCompleteInput 
                apiKey="NtdXMcSjbr4h__U2wEhaC7i-4wTlX71ofanOwpm5E3s" 
            ...
            {/* Butonul poate fi folosit dacă dorești o acțiune suplimentară, dar selecția din AutoComplete adaugă deja adresa
            <button type="button">Adaugă adresă</button> */}

            <h3>Adrese introduse</h3>
            {addresses.length === 0 && <p>Nicio adresă introdusă.</p>}
            <ul>
          ...
          {/* Afișare detalii rută și costuri */}
      ...
      {/* Transmiterea datelor către TollCalculator */}
      <TollCalculator
